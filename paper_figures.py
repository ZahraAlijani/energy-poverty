...
# Fig. 5: burden and income transitions bracketing tau_3 = 0.20 and tau_4 = 0.60.
B_L, B_U = 0.15, 0.25
Y_L, Y_U = 0.50, 0.70
TAU_B, TAU_Y = 0.20, 0.60
B_RANGE = (0.0, 0.4)
Y_RANGE = (0.0, 1.2)
...
def figure_debt():
    ...
    ax_i.step([0, TAU_D, TAU_D, 1], [0, 0, 1, 1], where="post",
              ...
              label=r"crisp $\mathbf{1}\{d\geq\tau_2\}$")
    ...
    ax_i.annotate(r"$\tau_2$", (TAU_D, 0.07), ha="left", fontsize=8)
...
def figure_expenses_income():
    ...
    for x, lbl, ha in ((B_L, "$b_L$", "right"), (B_U, "$b_U$", "left")):
    ...
    mesh = ax_i.pcolormesh(B, Y, I3, shading="gouraud", cmap="viridis",
                           vmin=0.0, vmax=1.0)
    # Boundary of the crisp deprivation region {b >= tau_3} and {y <= tau_4}.
    ax_i.plot([TAU_B, TAU_B], [Y_RANGE[0], TAU_Y], color="white", ls="--", lw=1.2)
    ax_i.plot([TAU_B, B_RANGE[1]], [TAU_Y, TAU_Y], color="white", ls="--", lw=1.2)
    ax_i.set_xlabel("$b(t)$")
    ax_i.set_ylabel("$y(t)$")
