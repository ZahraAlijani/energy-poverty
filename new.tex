...
The analysis so far has treated the self-correction rates $\lambda_h,\lambda_d,\lambda_b,\lambda_p$ as free parameters and shown, in Sections~\ref{sec:sim-trap} and~\ref{sec:sim-stable}, that the direct damping of the energy burden $\lambda_b$ is decisive for whether deprivation persists or fades. We now illustrate that this is not merely a theoretical possibility by calibrating the simplified system on Czech EU--SILC microdata and quantifying the empirical role of $\lambda$. The purpose is illustrative rather than predictive: with only two decades of annual observations the exercise cannot identify the full parameter set precisely, but it is sufficient to locate the observed system relative to the stability boundary and to show that the burden dimension is the binding one, exactly as the theory predicts.

\paragraph{Data and construction of the state series.}
We use household-level EU--SILC data for the Czech Republic covering 2005--2025, comprising roughly $8{,}500$ households per year. For each household the three fuzzy indicators of Section~\ref{sec:fuzzy} and the burden and poverty memberships are available directly; averaging them with the household cross-sectional weights yields a weighted annual prevalence for each dimension. We identify these aggregate intensities with the state variables of the model, so that $h(t)$ is the fuzzy heating-and-cooling indicator $\tilde I_1$, $d(t)$ the fuzzy debt indicator $\tilde I_2$, $b(t)$ the fuzzy energy-burden membership $\mu_b$, and $p(t)$ the fuzzy poverty membership $\mu_p$, each measured as a population share in $[0,1]$. The burden and poverty series are available from 2009 onward, giving $n=17$ annual points; they are shown in Figure~\ref{fig:empirical-lambda}(a). Poverty and, to a lesser extent, energy burden remain at high and persistent levels throughout, whereas thermal deprivation and arrears are low and react visibly to the COVID-19 period and the 2022--2023 energy-price shock.

\paragraph{Calibration.}
Interpreting the annual change of each series as a discrete approximation of its time derivative, we fit the simplified system~\eqref{eq:red_h}--\eqref{eq:red_p} with the sparsity pattern of the matrix $A$, adding a constant term that represents a persistent exogenous inflow and shifts the equilibrium away from the origin toward the observed levels. Each equation is estimated by ordinary least squares, and the self-correction rate of a dimension is read off the corresponding diagonal entry, $\lambda_i=-A_{ii}$. The estimated rates (per year) are
\[
\lambda_h\approx0.75,\qquad
\lambda_d\approx0.41,\qquad
\lambda_b\approx0.28,\qquad
\lambda_p\approx0.28,
\]
all positive, so every dimension is intrinsically mean-reverting. The eigenvalues of the fitted $A$ are $-0.80\pm0.14i$ and $-0.06\pm0.08i$; the dominant one has real part $\approx-0.06$. The observed Czech system is therefore stable but only marginally so: deprivation returns to its equilibrium very slowly, with a characteristic time of roughly $1/0.06\approx16$ years, which is the dynamical signature of a near-trap and is consistent with the well-documented persistence of energy poverty.
...
defines the critical rate $\lambda_b^*\approx0.11$, while the estimated
$\lambda_b\approx0.28$ places the observed system on the stable side but close
to the boundary.}
\label{fig:empirical-lambda}
\end{figure}

\paragraph{Impact of $\lambda_b$.}
To isolate the role of the burden dimension we vary $\lambda_b$ while holding the remaining entries of $A$ at their estimated values and track the dominant eigenvalue, Figure~\ref{fig:empirical-lambda}(b). It decreases monotonically in $\lambda_b$ and changes sign at a critical value $\lambda_b^*\approx0.11$: below it the reinforcing feedback from arrears and poverty overwhelms the weak damping of the burden and the zero-deprivation equilibrium becomes unstable, whereas above it the system converges. The estimated $\lambda_b\approx0.28$ lies above this threshold but within the same order of magnitude, so the observed margin of stability is thin and rests specifically on the burden channel. The same sweep applied to $\lambda_h$, $\lambda_d$, or $\lambda_p$ leaves the dominant eigenvalue negative over the entire plausible range, confirming that among the four rates it is $\lambda_b$ that governs the qualitative behavior. This is the empirical counterpart of the contrast between Figures~\ref{fig:unstable} and~\ref{fig:stable}: a structural weakening of energy-burden correction, for instance the withdrawal of affordability measures during a price shock, could move the real system across $\lambda_b^*$ into a persistent trap, whereas policies that raise $\lambda_b$ enlarge the stability margin.

\paragraph{Caveats.}
The calibration gives an order of magnitude, not a precise estimate. Four limitations matter. First, the series are short: seventeen annual points carry little information about the coefficients. Second, averaging households into national shares hides the differences between them, so the model describes an average household rather than a varied population. Third, the equations are linear and closed, and therefore leave out the external forces—energy prices, weather, policy changes—that visibly move the observed series; the fit has to attribute their effect to the internal feedbacks. Fourth, the cross-effect coefficients are weakly identified, because the four series rise and fall together and the regression cannot cleanly separate their mutual influences.

Two conclusions nevertheless survive, because both are qualitative rather than numerical. All self-correction rates are positive but small, so the system does return to equilibrium, only very slowly. And among the four rates, $\lambda_b$ is the one closest to the stability boundary. These are exactly the two features that the theory of Sections~\ref{sec:sim-trap} and~\ref{sec:sim-stable} predicts in advance. The data were free to contradict them and did not. The exercise therefore supports the mechanism developed above, even though it cannot measure it.
...
\url{\codeurl}
\end{center}
and consists of three Python modules. The module \texttt{FuzzyEnergyPoverty.py}
...
eigenvalues and index levels quoted in the text. The script
\texttt{empirical\_lambda.py} carries out the empirical calibration of
Section~\ref{sec:empirical-lambda}: it reads the Czech EU--SILC file
\texttt{dom\_all\_echudoba.RDS}, builds the weighted annual prevalence series,
estimates the self-correction rates $\lambda_i$, and produces
Figure~\ref{fig:empirical-lambda} together with the eigenvalues and the critical
rate $\lambda_b^*$ quoted in the text.
